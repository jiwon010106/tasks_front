    ...
    <nav className="navi bg-[#212121] w-1/5 h-full rounded-sm border border-gray-500 py-10 px-4 flex flex-col justify-between items-center">
      ...
      <ul className="menus">
        {navMenus.map((menu, idx) => (
          ...
          >
            <Link to={menu.to} className="flex gap4 items-center py-2 px-10">
              {menu.icon}
              {menu.label}
            </Link>
          </li>
      ...
      <div className="w-4/5">
        <button className="flex justify-center items-center gap-2 bg-gray-300 text-gray-900 py-3 px-4 rounded-sm w-full">
